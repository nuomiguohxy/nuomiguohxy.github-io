...
var Engine = (function(global) {
    ...
    /*  l这个函数调用一些初始化工作，特别是设置游戏必须的astTime 变量，这些工作只用
     * 做一次就够了
     */
    function init() {
        //reset();
    ...
    /* 额这个函数被 main 函数（我们的游戏主循环）调用，它本身调用所有的需要更新游戏角色
     * 数据的函数，取决于你怎样实现碰撞检测（
     * 意思是如何检测两个角色占据了同一个位置，
     * 比如你的角色死的时候），你可能需要在这里调用一个外的函数。现在我们已经把这里
    ...
    */
    function update(dt) {
        ...
        allEnemies.forEach(function(enemy){
            if(Math.abs(enemy.x-player.x)<enemy.size[0]-15 && Math.abs(enemy.y-player.y)<enemy.size[1]-50)
            {
                reset();
            }
    ...
    function render() {
        ...
        for (row = 0; row < numRows; row++) {
            for (col = 0; col < numCols; col++) {
                 ...
                 * 第二和个第三个分别是起始点的x和y坐标。我们用我们事先写好的资源管理工具来获取
